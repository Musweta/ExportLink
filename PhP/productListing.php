...
// Handle product creation (farmers)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_SESSION['role'] == 'farmer' && isset($_POST['create_product'])) {
...
try {
    $query = $_SESSION['role'] == 'farmer'
        ? "SELECT id, name, price, image_path FROM products WHERE farmer_id = ?"
        : "SELECT id, name, price, image_path FROM products";
    $stmt = $pdo->prepare($query);
    $stmt->execute([$_SESSION['role'] == 'farmer' ? $_SESSION['user_id'] : null]);
...
<div class="container mt-5">
    ...
    <?php if (isset($_GET['product_id']) && $_SESSION['role'] == 'importer'): ?>
        <?php
